...
def getDataSet(test_size):
    ...
    :return:
    """
    dataSet = pd.read_csv("dataset/iris.data").values.tolist()
    random.seed(12)
...
def createTree(dataSet, labels):
    ...
    if flagSeries:
        ...
        myTree[bestFeatLabel]['less'] = subTree

        # 递归处理大于当前划分点的子树
        subTree = createTree(gtDataSet, subLabels)
        myTree[bestFeatLabel]['greater'] = subTree
...
def classify(inputTree, featLables, testVec):
    ...
    for key in secondDict.keys():
        if testVec[featIndex] > float(firstStr[firstStr.index('=') + 1:]):
            if type(secondDict['greater']).__name__ == 'dict':
                classLabel = classify(secondDict['greater'], featLables, testVec)
            else:
                classLabel = secondDict['greater']
            return classLabel
        else:
            if type(secondDict['less']).__name__ == 'dict':
                classLabel = classify(secondDict['less'], featLables, testVec)
            else:
                classLabel = secondDict['less']
            return classLabel
...
#
#
# def classify(inputTree, featLables, x_test, label):
#     firstStr = list(inputTree.keys())[0]
#     secondDict = inputTree[firstStr]  # 树的分支，子集合Dict
#     seperate = firstStr.index(':')
#     featIndex = featLables.index(firstStr[seperate + 1:firstStr.index('=')])  # 获取决策树第一层在featLables中的位置
#     for key in secondDict.keys():
#         if x_test[featIndex] > float(firstStr[firstStr.index('=') + 1:]):
#             if type(secondDict['great']).__name__ == 'dict':
#                 classLabel = classify(secondDict['great'], featLables, x_test, label)
#             else:
#                 classLabel = secondDict['great']
#                 seperate3 = classLabel.index(':')
#                 classLabel = classLabel[seperate3 + 1:]
#             return classLabel
#         else:
#             if type(secondDict['less']).__name__ == 'dict':
#                 classLabel = classify(secondDict['less'], featLables, x_test, label)
#             else:
#                 classLabel = secondDict['less']
#                 seperate3 = classLabel.index(':')
#                 classLabel = classLabel[seperate3 + 1:]
...
#
#
# def calConfuMatrix(myTree, label, x_test, y_test, attribute):
#     matrix = {'Iris-setosa': {'Iris-setosa': 0, 'Iris-versicolor': 0, 'Iris-virginica': 0},
#               'Iris-versicolor': {'Iris-setosa': 0, 'Iris-versicolor': 0, 'Iris-virginica': 0},
#               'Iris-virginica': {'Iris-setosa': 0, 'Iris-versicolor': 0, 'Iris-virginica': 0}}
#     assert len(x_test) == len(y_test)
#     for i in range(len(y_test)):
#         predict = classify(myTree, attribute, x_test[i], label)  # 对测试数据进行测试，predict为预测的分类
#         actual = label[y_test[i]] # actual 为实际的分类
#         matrix[actual][predict] += 1  # 填充confusion matrix
#     return matrix